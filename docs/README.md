...
The primary survey raw responses are not publicly released to protect participant confidentiality. Processed analytical outputs and documentation are provided to support transparent inspection of the reported results. The external validation was used only for predictive robustness validation because the external dataset does not reproduce the same full SEM measurement structure as the primary survey.
